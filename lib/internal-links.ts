// AUTO-GENERATED — do not edit manually.
// Regenerate: npx tsx scripts/generate-links.ts

export type InternalLink = { href: string; title: string; anchor: string }

export const productInternalLinks: Record<string, InternalLink[]> = {
  "bpc-157": [
    {
      "href": "/products/tb-500",
      "title": "TB-500",
      "anchor": "Recovery"
    },
    {
      "href": "/stacks/looks-max",
      "title": "The Looks Max Stack",
      "anchor": "Recovery"
    },
    {
      "href": "/stacks/wolverine",
      "title": "The Wolverine Stack",
      "anchor": "Recovery"
    },
    {
      "href": "/stacks/weight-loss",
      "title": "The Metabolic Reset",
      "anchor": "Anti-Inflammatory"
    }
  ],
  "tb-500": [
    {
      "href": "/stacks/wolverine",
      "title": "The Wolverine Stack",
      "anchor": "Recovery"
    },
    {
      "href": "/products/bpc-157",
      "title": "BPC-157",
      "anchor": "Recovery"
    },
    {
      "href": "/stacks/looks-max",
      "title": "The Looks Max Stack",
      "anchor": "Recovery"
    },
    {
      "href": "/products/bacteriostatic-water",
      "title": "Bacteriostatic Water 10ml",
      "anchor": "Recovery"
    }
  ],
  "bacteriostatic-water": [
    {
      "href": "/products/tirzepatide-60mg",
      "title": "Tirzepatide 60mg",
      "anchor": "Reconstitution"
    },
    {
      "href": "/products/tirzepatide-30mg",
      "title": "Tirzepatide 30mg",
      "anchor": "Reconstitution"
    },
    {
      "href": "/products/tirzepatide-15mg-4-pack",
      "title": "Tirzepatide 15mg (4-Pack)",
      "anchor": "Reconstitution"
    },
    {
      "href": "/products/tirzepatide-20mg-5-pack",
      "title": "Tirzepatide 20mg (5-Pack)",
      "anchor": "Reconstitution"
    }
  ],
  "ghk-cu": [
    {
      "href": "/products/snap-8",
      "title": "SNAP-8",
      "anchor": "Skin"
    },
    {
      "href": "/products/snap-8-2-pack",
      "title": "SNAP-8 (2-Pack)",
      "anchor": "Skin"
    },
    {
      "href": "/stacks/longevity",
      "title": "The Longevity Stack",
      "anchor": "Skin"
    },
    {
      "href": "/stacks/looks-max",
      "title": "The Looks Max Stack",
      "anchor": "Skin"
    }
  ],
  "snap-8": [
    {
      "href": "/products/snap-8-2-pack",
      "title": "SNAP-8 (2-Pack)",
      "anchor": "Skin"
    },
    {
      "href": "/products/ghk-cu",
      "title": "GHK-Cu",
      "anchor": "Skin"
    },
    {
      "href": "/stacks/looks-max",
      "title": "The Looks Max Stack",
      "anchor": "Skin"
    },
    {
      "href": "/stacks/longevity",
      "title": "The Longevity Stack",
      "anchor": "Skin"
    }
  ],
  "snap-8-2-pack": [
    {
      "href": "/products/snap-8",
      "title": "SNAP-8",
      "anchor": "Skin"
    },
    {
      "href": "/products/ghk-cu",
      "title": "GHK-Cu",
      "anchor": "Skin"
    },
    {
      "href": "/stacks/looks-max",
      "title": "The Looks Max Stack",
      "anchor": "Skin"
    },
    {
      "href": "/stacks/longevity",
      "title": "The Longevity Stack",
      "anchor": "Skin"
    }
  ],
  "cjc-1295-ipamorelin": [
    {
      "href": "/products/igf-1-lr3",
      "title": "IGF-1 LR3",
      "anchor": "Muscle Growth"
    },
    {
      "href": "/products/retatrutide",
      "title": "Retatrutide",
      "anchor": "Fat Loss"
    },
    {
      "href": "/stacks/body-comp",
      "title": "The Body Sculptor",
      "anchor": "Sleep"
    },
    {
      "href": "/stacks/looks-max",
      "title": "The Looks Max Stack",
      "anchor": "Sleep"
    }
  ],
  "igf-1-lr3": [
    {
      "href": "/products/cjc-1295-ipamorelin",
      "title": "CJC-1295 / Ipamorelin",
      "anchor": "Muscle Growth"
    },
    {
      "href": "/stacks/body-comp",
      "title": "The Body Sculptor",
      "anchor": "Anabolic"
    },
    {
      "href": "/products/tb-500",
      "title": "TB-500",
      "anchor": "Systemic Tissue Regeneration"
    },
    {
      "href": "/stacks/looks-max",
      "title": "The Looks Max Stack",
      "anchor": "Comprehensive skin, collagen & cellular optimization"
    }
  ],
  "semaglutide": [
    {
      "href": "/products/cagrilintide-semaglutide",
      "title": "Cagrilintide + Semaglutide",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/semaglutide-10mg",
      "title": "Semaglutide 10mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/semaglutide-15mg",
      "title": "Semaglutide 15mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide",
      "title": "Tirzepatide",
      "anchor": "Weight Loss"
    }
  ],
  "semaglutide-10mg": [
    {
      "href": "/products/retatrutide-30mg",
      "title": "Retatrutide 30mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/semaglutide",
      "title": "Semaglutide",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/semaglutide-15mg",
      "title": "Semaglutide 15mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide",
      "title": "Tirzepatide",
      "anchor": "Weight Loss"
    }
  ],
  "semaglutide-15mg": [
    {
      "href": "/products/retatrutide-60mg",
      "title": "Retatrutide 60mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/semaglutide-10mg",
      "title": "Semaglutide 10mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide-15mg-10-pack",
      "title": "Tirzepatide 15mg (10-Pack)",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide-60mg",
      "title": "Tirzepatide 60mg",
      "anchor": "Weight Loss"
    }
  ],
  "tirzepatide": [
    {
      "href": "/products/retatrutide",
      "title": "Retatrutide",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/semaglutide",
      "title": "Semaglutide",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/semaglutide-15mg",
      "title": "Semaglutide 15mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide-20mg-5-pack",
      "title": "Tirzepatide 20mg (5-Pack)",
      "anchor": "Weight Loss"
    }
  ],
  "tirzepatide-15mg-4-pack": [
    {
      "href": "/products/tirzepatide-15mg-10-pack",
      "title": "Tirzepatide 15mg (10-Pack)",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide-20mg-5-pack",
      "title": "Tirzepatide 20mg (5-Pack)",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide-30mg",
      "title": "Tirzepatide 30mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/retatrutide",
      "title": "Retatrutide",
      "anchor": "Weight Loss"
    }
  ],
  "tirzepatide-15mg-10-pack": [
    {
      "href": "/products/tirzepatide-15mg-4-pack",
      "title": "Tirzepatide 15mg (4-Pack)",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide-20mg-5-pack",
      "title": "Tirzepatide 20mg (5-Pack)",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide-60mg",
      "title": "Tirzepatide 60mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/retatrutide",
      "title": "Retatrutide",
      "anchor": "Weight Loss"
    }
  ],
  "tirzepatide-20mg-5-pack": [
    {
      "href": "/products/tirzepatide-15mg-10-pack",
      "title": "Tirzepatide 15mg (10-Pack)",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide-15mg-4-pack",
      "title": "Tirzepatide 15mg (4-Pack)",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/retatrutide",
      "title": "Retatrutide",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide",
      "title": "Tirzepatide",
      "anchor": "Weight Loss"
    }
  ],
  "tirzepatide-30mg": [
    {
      "href": "/products/tirzepatide-15mg-4-pack",
      "title": "Tirzepatide 15mg (4-Pack)",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide",
      "title": "Tirzepatide",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide-60mg",
      "title": "Tirzepatide 60mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/retatrutide",
      "title": "Retatrutide",
      "anchor": "Weight Loss"
    }
  ],
  "tirzepatide-60mg": [
    {
      "href": "/products/retatrutide-60mg",
      "title": "Retatrutide 60mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide",
      "title": "Tirzepatide",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide-15mg-10-pack",
      "title": "Tirzepatide 15mg (10-Pack)",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide-15mg-4-pack",
      "title": "Tirzepatide 15mg (4-Pack)",
      "anchor": "Weight Loss"
    }
  ],
  "retatrutide": [
    {
      "href": "/products/retatrutide-10mg",
      "title": "Retatrutide 10mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/retatrutide-30mg",
      "title": "Retatrutide 30mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/retatrutide-60mg",
      "title": "Retatrutide 60mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide",
      "title": "Tirzepatide",
      "anchor": "Weight Loss"
    }
  ],
  "retatrutide-10mg": [
    {
      "href": "/products/retatrutide",
      "title": "Retatrutide",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/retatrutide-30mg",
      "title": "Retatrutide 30mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/retatrutide-60mg",
      "title": "Retatrutide 60mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/cagrilintide-semaglutide",
      "title": "Cagrilintide + Semaglutide",
      "anchor": "Weight Loss"
    }
  ],
  "retatrutide-30mg": [
    {
      "href": "/products/retatrutide-60mg",
      "title": "Retatrutide 60mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/retatrutide",
      "title": "Retatrutide",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/retatrutide-10mg",
      "title": "Retatrutide 10mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide",
      "title": "Tirzepatide",
      "anchor": "Weight Loss"
    }
  ],
  "retatrutide-60mg": [
    {
      "href": "/products/retatrutide",
      "title": "Retatrutide",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/retatrutide-10mg",
      "title": "Retatrutide 10mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/retatrutide-30mg",
      "title": "Retatrutide 30mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/semaglutide-10mg",
      "title": "Semaglutide 10mg",
      "anchor": "Weight Loss"
    }
  ],
  "cagrilintide-semaglutide": [
    {
      "href": "/products/semaglutide",
      "title": "Semaglutide",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/semaglutide-15mg",
      "title": "Semaglutide 15mg",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/tirzepatide",
      "title": "Tirzepatide",
      "anchor": "Weight Loss"
    },
    {
      "href": "/products/semaglutide-10mg",
      "title": "Semaglutide 10mg",
      "anchor": "Weight Loss"
    }
  ],
  "epithalon": [
    {
      "href": "/stacks/longevity",
      "title": "The Longevity Stack",
      "anchor": "Anti-Aging"
    },
    {
      "href": "/stacks/looks-max",
      "title": "The Looks Max Stack",
      "anchor": "Anti-Aging"
    },
    {
      "href": "/products/nad-plus",
      "title": "NAD+",
      "anchor": "Anti-Aging"
    },
    {
      "href": "/stacks/body-comp",
      "title": "The Body Sculptor",
      "anchor": "Sleep"
    }
  ],
  "nad-plus": [
    {
      "href": "/stacks/longevity",
      "title": "The Longevity Stack",
      "anchor": "Anti-Aging"
    },
    {
      "href": "/products/epithalon",
      "title": "Epithalon",
      "anchor": "Anti-Aging"
    },
    {
      "href": "/stacks/looks-max",
      "title": "The Looks Max Stack",
      "anchor": "Anti-Aging"
    },
    {
      "href": "/products/ghk-cu",
      "title": "GHK-Cu",
      "anchor": "Anti-Aging"
    }
  ]
}

export const stackInternalLinks: Record<string, InternalLink[]> = {
  "looks-max": [
    {
      "href": "/products/bpc-157",
      "title": "BPC-157",
      "anchor": "Recovery"
    },
    {
      "href": "/products/cjc-1295-ipamorelin",
      "title": "CJC-1295 / Ipamorelin",
      "anchor": "Sleep"
    },
    {
      "href": "/products/epithalon",
      "title": "Epithalon",
      "anchor": "Anti-Aging"
    },
    {
      "href": "/products/ghk-cu",
      "title": "GHK-Cu",
      "anchor": "Skin"
    }
  ],
  "body-comp": [
    {
      "href": "/products/cagrilintide-semaglutide",
      "title": "Cagrilintide + Semaglutide",
      "anchor": "Dual-Mechanism Satiety Stack"
    },
    {
      "href": "/products/cjc-1295-ipamorelin",
      "title": "CJC-1295 / Ipamorelin",
      "anchor": "Sleep"
    },
    {
      "href": "/products/igf-1-lr3",
      "title": "IGF-1 LR3",
      "anchor": "Anabolic"
    },
    {
      "href": "/products/retatrutide",
      "title": "Retatrutide",
      "anchor": "Triple Agonist — Maximum Fat Loss"
    }
  ],
  "wolverine": [
    {
      "href": "/products/bpc-157",
      "title": "BPC-157",
      "anchor": "Recovery"
    },
    {
      "href": "/products/tb-500",
      "title": "TB-500",
      "anchor": "Recovery"
    },
    {
      "href": "/stacks/body-comp",
      "title": "The Body Sculptor",
      "anchor": "GH optimization + direct anabolism for elite body composition"
    },
    {
      "href": "/stacks/looks-max",
      "title": "The Looks Max Stack",
      "anchor": "Comprehensive skin, collagen & cellular optimization"
    }
  ],
  "weight-loss": [
    {
      "href": "/products/cagrilintide-semaglutide",
      "title": "Cagrilintide + Semaglutide",
      "anchor": "GLP-1"
    },
    {
      "href": "/products/retatrutide",
      "title": "Retatrutide",
      "anchor": "GLP-1"
    },
    {
      "href": "/products/retatrutide-10mg",
      "title": "Retatrutide 10mg",
      "anchor": "GLP-1"
    },
    {
      "href": "/products/semaglutide",
      "title": "Semaglutide",
      "anchor": "GLP-1"
    }
  ],
  "longevity": [
    {
      "href": "/products/epithalon",
      "title": "Epithalon",
      "anchor": "Anti-Aging"
    },
    {
      "href": "/products/ghk-cu",
      "title": "GHK-Cu",
      "anchor": "Skin"
    },
    {
      "href": "/products/nad-plus",
      "title": "NAD+",
      "anchor": "Anti-Aging"
    },
    {
      "href": "/stacks/body-comp",
      "title": "The Body Sculptor",
      "anchor": "GH optimization + direct anabolism for elite body composition"
    }
  ]
}
